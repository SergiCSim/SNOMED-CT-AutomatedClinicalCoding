...
# Set up the basic configuration for logging using loguru
...
def make_clean_annotations():
    ...
    for i in annotations.index:
        try:
            lst.append(snomed[annotations.loc[i, "concept_id"]])
        except:
            continue
...
def make_abbreviations():
    ...
    for i, abbr_meaning in enumerate(abbr_meanings):
        if (i + 1) % 10 == 0:
            print(str(i) + '/' + str(len(abbr_meanings)))
        else:
            print(str(i) + '/' + str(len(abbr_meanings)), end=' ')
...
def make_unigrams():
    ...
    # Read the CSV in chunks and update the Counter
    chunk_size = 1000  # Adjust this size based on your memory capacity
    total_rows = pd.read_csv(raw_directory / "discharge.csv.gz", usecols=["text"]).shape[0]
    total_chunks = (total_rows // chunk_size) + 1
    for i, chunk in enumerate(pd.read_csv(raw_directory / "discharge.csv.gz", usecols=["text"], chunksize=chunk_size)):
        print('Processing chunk ' + str(i) + '/' + str(total_chunks))
        chunk_text = "\n".join(chunk["text"]).lower()
        all_chunk_text = " ".join(chunk_text.split())
        text_counter.update(all_chunk_text.split())
    ...
    # Get SNOMED synonyms and filter based on thresholds
    snomed_syns, sno_fsn, z_ = get_snomed_synonyms(min_len=1, max_len=1, fsn_only=True)
    unigram_dict_20k = {k: v for k, v in snomed_syns.items() if text_counter[k[1]] < th_20k}
    ...
    unigram_dict_3k = {k: v for k, v in snomed_syns.items() if text_counter[k[1]] < th_3k}
