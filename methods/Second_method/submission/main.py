...
from score_mixing import mix_scores
...
def main(
    ...
    tdf = second_stage(mentions_df, sap_checkpoint, topk=10) #topk=1
    #tdf.to_csv('interm.csv', index=False)
    logger.debug(f"{tdf.shape=}")
    logger.debug(f"{tdf.head()}")
    
    tdf.to_csv('interm.csv')
    tdf = pd.read_csv('interm.csv')
    
    # Mix probs
    print('Mixing scores...')
    tdf = mix_scores(tdf, train_annotations_path, train_notes_path)
    print('Scores mixed')
